"""Bloque I.10 — igual que T_I24, mas las medias de cada variable por conglomerado.

Cuarta de la serie y superconjunto de la tercera: mismo universo, mismo k
recalculado en cada hoja, y al pie de cada hoja DOS tablas de medias por
conglomerado, una por periodo:
    ...
    en niveles     las siete variables en unidades legibles, mas n, participacion
                   ...
                   Productividad, remuneracion media y FBCF por ocupado van como
                   indice con la economia = 100, que es lo unico comparable entre
                   2003 y 2023.
    en puntajes z  los centroides con que trabaja el algoritmo: las variables ya
                   transformadas y estandarizadas, ponderadas por empleo si la
                   especificacion pondera. Es el equivalente de la hoja
                   `estadisticas_cluster` de los libros 3a y 3b.

Van al pie de la hoja y no debajo de cada bloque porque el bloque del periodo 2
ocupa las columnas 7 a 11: una tabla de medias no cabe en las cinco columnas del
periodo 1. Quedan una al lado de la otra, en el mismo orden que los bloques.

Lo demas es identico a py/31, de donde hereda las dos diferencias con las dos
primeras planillas de la serie:
...
Salida:   datos/output/2026.08.13 T_I25 clasificacion con medias por conglomerado.xlsx
"""
...
KMAX = 8
SALIDA = rutas.OUT / "2026.08.13 T_I25 clasificacion con medias por conglomerado.xlsx"
...
def main() -> None:
    ...
    ws.cell(1, 1, "T_I25 — como T_I24, mas las medias de cada variable por conglomerado"
            ).font = Font(bold=True, size=13)
    ...
    ws.cell(3, 1, "Cuarta de la serie. T_I22 clasifica solo las 62 y 101 que la UAM dejo adentro; "
                  "T_I23, la COU completa; las dos con k = 4 impuesto. T_I24 es esta misma sin "
                  "las tablas de medias."
            ).font = Font(italic=True, size=9)
    ws.cell(4, 1, "Al pie de cada hoja, dos tablas por periodo: medias en niveles (legibles) y "
                  "centroides en puntajes z (los que minimiza el algoritmo)."
            ).font = Font(italic=True, size=9)
